...
const props = withDefaults(
  ...
  {
    title: "音色库",
    subtitle: "管理克隆音色与样本，失效样本会明确标注且不可试听。",
    actionText: "自定义声音克隆",
    showSearch: false,
  },
);
...
const list = useCursorList<VoiceResource>((cursor) =>
  listVoiceResources(
    { scope: scope.value, cursor, limit: 18 },
    { noCache: true },
  ),
);
...
function canPreviewVoice(item: VoiceResource) {
  return Boolean(item.audioUrl?.trim()) && item.sampleMissing !== true;
}

function getPreviewUnavailableReason(item: VoiceResource) {
  if (item.sampleMissing) {
    return "样本文件缺失，无法试听。";
  }
  if (!item.audioUrl?.trim()) {
    return "当前音色没有可用样本，无法试听。";
  }
  return "";
}

function isRenderableVoice(item: VoiceResource) {
  if (typeof item.canUseForRender === "boolean") return item.canUseForRender;
  if (item.cloneStatus !== "ready") return false;
  return Boolean(item.providerVoice || item.audioUrl?.trim());
}

function getRenderUnavailableReason(item: VoiceResource) {
  if (item.renderUnavailableReason?.trim()) return item.renderUnavailableReason;
  if (item.cloneStatus === "processing") return "克隆处理中，暂不可用于生成。";
  if (item.cloneStatus === "failed") return "克隆失败，暂不可用于生成。";
  if (!isRenderableVoice(item)) return "当前音色暂不可用于生成。";
  return "";
}

function onPlayVoice(item: VoiceResource) {
  if (!canPreviewVoice(item)) {
    message.warning(getPreviewUnavailableReason(item));
    return;
  }
  player.toggle(item.id, item.audioUrl);
}
  ...
  try {
    ...
    if (item.provider === "local-upload") {
      const reason = item.cloneError
        ...
        : "可先作为本地样本使用";
      message.warning(
        `音频已保存到音色库；模型克隆未通过，已保存为本地样本。${reason}`,
      );
      return;
    }
  ...
  <main ref="scrollRef" class="resource-page">
    ...
    <n-spin :show="list.loading.value">
      <n-empty
        ...
        description="暂无音色资源，请先上传样本或克隆声音"
      ...
      <div v-else class="voice-grid">
        <VoiceResourceCard
          ...
          :show-play="canPreviewVoice(item)"
          :play-unavailable-reason="getPreviewUnavailableReason(item)"
          :render-ready="isRenderableVoice(item)"
          :render-unavailable-reason="getRenderUnavailableReason(item)"
          @update:selected="toggleSelected(item.id, $event)"
          @rename="rename(item, $event)"
          @delete="requestDelete([item.id])"
          @play="onPlayVoice(item)"
        />
      </div>
