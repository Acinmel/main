/**
 * 在加载其它模块前读入 .env，使 FileInterceptor 等装饰器能读到 TRANSCRIBE_MEDIA_MAX_BYTES。
 *
...
if (inDocker) {
  ...
  // 不得 override：docker compose 已把 FFMPEG_BIN、PORT 等注入 process.env；
  // backend/.env 里常见的 127.0.0.1、Windows 路径会误入容器。
